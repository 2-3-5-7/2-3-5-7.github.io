...
static int nsyms = 0;
static Elf64_Sym *elf64_syms = NULL;
...
static int fill_tab(int fd, const Elf64_Shdr *shdr, char **result)
{
	int r;
...
static void init()
{
	...
	xread(fd, &elf64_ehdr, sizeof(elf64_ehdr), location);

	if (lseek(fd, elf64_ehdr.e_shoff+elf64_ehdr.e_shstrndx*elf64_ehdr.e_shentsize, SEEK_SET)
	    != elf64_ehdr.e_shoff+elf64_ehdr.e_shstrndx*elf64_ehdr.e_shentsize) {
		fprintf(stderr, "%slseek: %s\n", location, strerror(errno));
		exit(errno);
	}
	...
	/* Get shstrtab */
	xread(fd, &elf64_shdr, sizeof(elf64_shdr), location);

	shstrtab = (char *)malloc(elf64_shdr.sh_size);
	if (lseek(fd, elf64_shdr.sh_offset, SEEK_SET) != elf64_shdr.sh_offset) {
		fprintf(stderr, "%slseek: %s\n", location, strerror(errno));
		exit(errno);
	}

	xread(fd, shstrtab, elf64_shdr.sh_size, location);

	if (lseek(fd, elf64_ehdr.e_shoff, SEEK_SET)
	    != elf64_ehdr.e_shoff) {
		fprintf(stderr, "%slseek: %s\n", location, strerror(errno));
		exit(errno);
	}
	...
	while (!done) {
		r = xread(fd, &elf64_shdr, sizeof(elf64_shdr), location);
		if (strcmp(&shstrtab[elf64_shdr.sh_name], ".strtab") == 0) { 
			slen = fill_tab(fd, &elf64_shdr, &strtab);
		}
		if (elf64_shdr.sh_type == SHT_SYMTAB) {
			r = fill_tab(fd, &elf64_shdr, (char**)&elf64_syms);
			nsyms = r/sizeof(Elf64_Sym);
		}
		++n;
		done = elf64_syms != NULL && strtab != NULL;
	}
	close(fd);
...
void __cyg_profile_func_enter(void *this_fn, void *call_site)
{
	...
	for (i = 0; i < nsyms; ++i) {
		if (ELF64_ST_TYPE(elf64_syms[i].st_info) == STT_FUNC &&
		    elf64_syms[i].st_value == (Elf64_Addr)this_fn) {
			fprintf(logf, " %s (%p)\n",
			       &strtab[elf64_syms[i].st_name], this_fn);
			if (depth + 1 < sizeof(call_chain)/sizeof(call_chain[0]))
				call_chain[depth+1] = &strtab[elf64_syms[i].st_name];
		}
	}
